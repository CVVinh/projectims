...
export function export_json_to_excel({
    ...
    // កំណត់ ក្បាល Excel
    var fontRoboto = 'Roboto'
    const columnCount = XLSX.utils.decode_range(ws['!ref'])
    for (var i = 0; i <= columnCount.e.c; ++i) {
        ws[`${XLSX.utils.encode_col(i)}1`].s = FormatExcel.Header
    }
    ...
    for (var i = 0; i <= columnCount.e.c; i++) {
        for (var j = 1; j < data.length; j++) {
            const headerCell = XLSX.utils.encode_cell({ r: j, c: i });
            if(i == 1) sheet[headerCell].s = FormatExcel.ElementContent1;
